...
export class AdministradorDeArchivosController {
  ...
  async CargarArchivoServicio(
    ...
    const filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosServicios);
  ...
  private ObtenerArchivosPorTipo(tipo: number) {
    ...
    switch (tipo) {
      // amusement
      case 1:
        filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosServicios);
        break;
      case 2:
